...
directory = "Letters"
curr_path = os.getcwd()
save_path = os.path.join(curr_path, directory)

# Create the folder "Letters" if it doesn't exist
...
def write_to_file(country, email, name, title):
    ...
    # No way to contact them through email
    if email == "":
        return
    ...
    # File names in windows cannot contain the chars in the unacceptable_chars list
    for c in email:
        if c in unacceptable_chars:
            return
    ...
    with open(completeName, "w", encoding="utf-8") as f:
        ...
        # Format the file content appropriately
...
for country in countries_as_json:
    for person in country['contactInformation']:
        # Create a text document for each government official
...
# Set up the sender email (from email)
email_from = input("Enter the sender's email: ")

# Define the App password
...
# Connect to the server outside the send_emails function to avoid opening & closing
...
# Iterate over the files in the directory where the letters are stored
for f in files:
    # Pause for 3 minutes every 50 email pushes to ensure
    # the Google SMTP server does not reject our requests
    if i % 50 == 0 and i != 0:
        time.sleep(180)
